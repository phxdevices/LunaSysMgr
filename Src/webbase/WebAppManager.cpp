...
#include <palmwebview.h> // IME support
#include <pthread.h>
#include <string>
	...
	, m_inSimulatedMouseEvent(false)
{
    ...
    m_QCoreApplication = new QCoreApplication(argc, (char **)argv);
	sInstance = this;
...
{
	WebAppBase* app = findApp(procId);
	if (app) {
		if (app->page()->processId() == procId) {
...
{
	WebAppBase* app = findApp(processId);
	if (app)
		app->page()->inspect();
}
...
bool WebAppManager::isAppRunning(const std::string& appId)
{
	...
	for(; itStart != itEnd; itStart++) {
		if(0 == (*itStart)->appId().compare(appId)) {
			return true;
		}
	}
...
void WebAppManager::onListOfRunningAppsRequest(bool includeSysApps)
{
	...
	for ( std::vector<ProcessInfo>::const_iterator it=apps.begin(); it != apps.end(); it++ )
	{
		...
		if (!o)
			continue;
		json_object_object_add(o, "id", json_object_new_string(it->appId.c_str()));
		json_object_object_add(o, "processid", json_object_new_string(it->processId.c_str()));
                                             ...
                                             bool ignoreLowMemory)
{
	...
	// Low Memory handling
	if (!ignoreLowMemory && preventAppUnderLowMemory(appId, winType, desc)) {
		errorCode = SystemUiController::InternalError;
		g_warning("%s: Low memory condition Not allowing card app for appId: %s", __PRETTY_FUNCTION__, appId.c_str());
	...
	if (G_UNLIKELY(Settings::LunaSettings()->perfTesting)) {
		g_message("SYSMGR PERF: APP START appid: %s, processid: %s, type: %s, time: %d",
				  appId.c_str(), procId.c_str(),
				  WebAppFactory::nameForWindowType(winType).c_str(),
				  Time::curTimeMs());
	}

	if (app) {
		WebPage* page = new WebPage(url.c_str(), winType != Window::Type_None);
		app->setAppDescription(desc);

		static_cast<ProcessBase*>(page)->setProcessId(procId);
		static_cast<ProcessBase*>(page)->setAppId(appId);
		static_cast<ProcessBase*>(page)->setLaunchingAppId(launchingAppId);
		static_cast<ProcessBase*>(page)->setLaunchingProcessId(launchingProcId);
...
WebAppBase* WebAppManager::launchWithPageInternal(WebPage* page, Window::Type winType, ApplicationDescription* parentDesc)
{
	if (preventAppUnderLowMemory(page->appId(), winType, parentDesc)) {
		g_warning("%s: Low memory condition Not allowing card app for appId: %s", __PRETTY_FUNCTION__, page->appId().c_str());
		// not enough memory. try to free up some memory and notify the user to close cards
		sendAsyncMessage(new ViewHost_AppLaunchPreventedUnderLowMemory());
	...
	if (app) {
		...
		static_cast<ProcessBase*>(page)->setProcessId(processId);
...
void WebAppManager::closeAppInternal(WebAppBase* app)
{
	...
	if( ( app->isWindowed() && Window::Type_Card == static_cast<WindowedWebApp*>(app)->windowType() )
		...
		&& (app->page()->url() == 0 || !strstr( app->page()->url(), "nocache=true" ))
	...
	if( appsToLaunchAtBoot.find(appId) == appsToLaunchAtBoot.end() )
	{
		...
		if (windowedAppCount == 0 && appToClose
		        && appsToKeepAliveUntilMemPressure.find(appId) == appsToKeepAliveUntilMemPressure.end() ) {
			g_message("Closing headless app with no windows: %s", appId.c_str());
			delete appToClose;
		}

	}

	if( appsToKeepAliveUntilMemPressure.find(appId) != appsToKeepAliveUntilMemPressure.end() ) {
		g_message("Keeping %s headless alive ...", appId.c_str() );
	}
}
...
void WebAppManager::setOrientationInternal(Event::Orientation orient)
{
	...
	for (AppWindowMap::iterator it = m_appWinMap.begin();
		...
		if (app && app->isWindowed()) {
			...
			if (windowedApp->windowType() == Window::Type_Emulated_Card) {
				CardWebApp* cardWebApp = static_cast<CardWebApp*>(windowedApp);
				orient = cardWebApp->appOrientationFor(orient);
			}
...
WebAppBase* WebAppManager::findApp(const std::string& processId)
{
...
void WebAppManager::restartHeadLessBootApps()
{
	...
	for (AppList::const_iterator it = appsToRelaunch.begin();
		...
		appInfo.id   = (*it)->page()->appId();
		(*it)->page()->client()->getAppDescription()->getAppDescriptionString(appInfo.desc);

		relaunchList.push_back(AppDescUrlPair(appInfo, (*it)->page()->url()));
...
void WebAppManager::webPageAdded(WebPage* page)
{
	...
	m_appPageMap.insert(AppIdWebPagePair(page->appId(), page));
}

void WebAppManager::shellPageAdded(WebPage* page)
{
	if (page && m_shellPageMap.find(page->appId()) == m_shellPageMap.end()) {
		m_shellPageMap[page->appId()] = page;
	}
}

void WebAppManager::webPageRemoved(WebPage* page)
{
	if (!m_deletingPages) {
...
{
    EMPTY_SCHEMA_RETURN(handle, message);
...
{
    // {"enable": boolean, "reset": integer, "dump": boolean}
    VALIDATE_SCHEMA_AND_RETURN(lsHandle,
                               message,
                               SCHEMA_3(REQUIRED(enable, boolean), REQUIRED(reset, integer), REQUIRED(dump, boolean)));
...
void WebAppManager::pastedFromClipboard(const std::string& appId)
{
...
WebPage* WebAppManager::takeShellPageForApp(const std::string& appId)
{
...
void WebAppManager::addHeadlessAppToWatchList(WebAppBase* app)
{
	...
	g_message("WATCH: Adding app to watch list: %s",  app->appId().c_str());
...
void WebAppManager::removeHeadlessAppFromWatchList(WebAppBase* app)
{
	...
	g_message("WATCH: Removing app from watch list: %s", app->appId().c_str());
...
bool WebAppManager::headlessAppWatchCallback()
{
	...
	for (AppLaunchTimeMap::iterator it = m_headlessAppLaunchTimeMap.begin();
		...
		g_message("WATCH: Checking app: %s", app->appId().c_str());
		...
		std::string appId = app->page()->appId();
...
void WebAppManager::reportAppLaunched(const std::string& appId, const std::string& processId)
{
	...
	json_object_object_add(json, (char*) "appId", json_object_new_string(appId.c_str()));
	json_object_object_add(json, (char*) "processId", json_object_new_string(processId.c_str()));
...
void WebAppManager::reportAppClosed(const std::string& appId, const std::string& processId)
{
	if (!getStatsServiceHandle())
		return;
